...
class BaseWatchdog(BaseModel):
		...
		def make_unique_handler(actual_handler):
			async def unique_handler(event):
				...
				try:
					...
					# Skip CDP repair when using cross-browser driver (Playwright)
					if getattr(browser_session, '_driver', None):
						raise
	...
	def attach_to_session(self) -> None:
		...
		from browser_ai.browser import events
	...
	def __del__(self) -> None:
		...
		try:
			...
			from browser_ai.utils import logger
