...
from browser_ai.browser.cloud.cloud import CloudBrowserAuthError, CloudBrowserClient, CloudBrowserError
...
from browser_ai.browser.cloud.views import CloudBrowserParams, CreateBrowserRequest, ProxyCountryCode
...
from browser_ai.browser.views import BrowserStateSummary, TabInfo
from browser_ai.dom.views import DOMRect, EnhancedDOMTreeNode, TargetInfo
from browser_ai.observability import observe_debug
from browser_ai.utils import _log_pretty_url, create_task_with_error_handling, is_new_tab_page

if TYPE_CHECKING:
	from browser_ai.actor.page import Page
	from browser_ai.browser.demo_mode import DemoMode
...
class BrowserSession(BaseModel):
	...
	def __init__(
		...
		max_iframes: int | None = None,
		max_iframe_depth: int | None = None,
		# Cross-browser driver (e.g. PlaywrightDriver); when set, CDP is not used
		...
		# Set _driver before super().__init__() so model_post_init sees it and skips CDP handler registration
		object.__setattr__(self, '_driver', driver)

		# Initialize the Pydantic model (model_post_init runs inside here and checks _driver)
		super().__init__(
	...
	# Optional cross-browser driver (Playwright); when set, CDP is not used
	_driver: Any = PrivateAttr(default=None)  # BrowserDriver protocol
	...
	def logger(self) -> Any:
		...
		# 	self._logger = logging.getLogger(f'browser_ai.{self}')
		return logging.getLogger(f'browser_ai.{self}')
	...
	async def reset(self) -> None:
		...
		# Close cross-browser driver first (Playwright)
		if self._driver:
			try:
				await self._driver.close()
			except Exception as e:
				self.logger.debug('Error closing driver during reset: %s', e)
			object.__setattr__(self, '_driver', None)
	...
	def model_post_init(self, __context) -> None:
		...
		from browser_ai.browser.watchdog_base import BaseWatchdog
		...
		# When using Playwright/cross-browser driver, do NOT register CDP-based handlers for navigation/focus — driver handles them
	...
	async def kill(self) -> None:
		...
		from browser_ai.browser.events import SaveStorageStateEvent
	...
	async def on_BrowserStartEvent(self, event: BrowserStartEvent) -> dict[str, str]:
		...
		# Cross-browser driver path (Playwright): skip CDP launch and connect
		if self._driver:
			await self._driver.start(self)
			self.event_bus.dispatch(BrowserConnectedEvent(cdp_url='playwright'))
			return {'cdp_url': 'playwright'}
	...
	async def on_NavigateToUrlEvent(self, event: NavigateToUrlEvent) -> None:
		...
		if self._driver:
			return  # Playwright/cross-browser driver handles navigation
	...
	async def on_AgentFocusChangedEvent(self, event: AgentFocusChangedEvent) -> None:
		...
		if self._driver:
			return  # No CDP focus switching for Playwright driver
	...
	def cdp_client(self) -> CDPClient:
		...
		if self._driver:
			raise RuntimeError('CDP is not available when using a cross-browser driver (Playwright). Use CDP backend for CDP-specific features.')
	...
	async def new_page(self, url: str | None = None) -> 'Page':
		...
		from browser_ai.actor.page import Page as Target
	...
	async def get_pages(self) -> list['Page']:
		...
		# Import here to avoid circular import
		from browser_ai.actor.page import Page as PageActor
	...
	async def get_or_create_cdp_session(self, target_id: TargetID | None = None, focus: bool = True) -> CDPSession:
		...
		"""
		if self._driver:
			raise RuntimeError('CDP session is not available when using a cross-browser driver (Playwright). Use CDP backend for CDP-specific features.')
	...
	async def get_browser_state_summary(
		...
		# Cross-browser driver path (e.g. Playwright)
		if self._driver:
			result = await self._driver.get_browser_state_summary(
				include_screenshot=include_screenshot,
				include_recent_events=include_recent_events,
			)
			self._cached_browser_state_summary = result
			self.update_cached_selector_map(result.dom_state.selector_map)
			return result
	...
	async def attach_all_watchdogs(self) -> None:
		...
		# Cross-browser driver: only attach driver's event handlers (no CDP watchdogs)
		if self._driver:
			self._driver.attach_handlers(self)
			self._watchdogs_attached = True
			self.logger.debug('Attached cross-browser driver handlers (no CDP watchdogs)')
			return

		from browser_ai.browser.watchdogs.aboutblank_watchdog import AboutBlankWatchdog

		# from browser_ai.browser.crash_watchdog import CrashWatchdog
		from browser_ai.browser.watchdogs.default_action_watchdog import DefaultActionWatchdog
		from browser_ai.browser.watchdogs.dom_watchdog import DOMWatchdog
		from browser_ai.browser.watchdogs.downloads_watchdog import DownloadsWatchdog
		from browser_ai.browser.watchdogs.local_browser_watchdog import LocalBrowserWatchdog
		from browser_ai.browser.watchdogs.permissions_watchdog import PermissionsWatchdog
		from browser_ai.browser.watchdogs.popups_watchdog import PopupsWatchdog
		from browser_ai.browser.watchdogs.recording_watchdog import RecordingWatchdog
		from browser_ai.browser.watchdogs.screenshot_watchdog import ScreenshotWatchdog
		from browser_ai.browser.watchdogs.security_watchdog import SecurityWatchdog
		from browser_ai.browser.watchdogs.storage_state_watchdog import StorageStateWatchdog
	...
	async def connect(self, cdp_url: str | None = None) -> Self:
		...
		try:
			...
			from browser_ai.browser.session_manager import SessionManager
			...
			from browser_ai.utils import is_new_tab_page
	...
	async def navigate_to(self, url: str, new_tab: bool = False) -> None:
		...
		"""
		from browser_ai.browser.events import NavigateToUrlEvent
