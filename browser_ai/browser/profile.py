...
from browser_ai.browser.cloud.views import CloudBrowserParams
from browser_ai.config import CONFIG
from browser_ai.utils import _log_pretty_path, logger
...
# Using constants from central location in browser_ai.config
...
class BrowserProfile(BrowserConnectArgs, BrowserLaunchPersistentContextArgs, BrowserLaunchArgs, BrowserNewContextArgs):
	...
	proxy: ProxySettings | None = Field(
		default=None,
		description='Proxy settings. Use browser_ai.browser.profile.ProxySettings(server, bypass, username, password)',
	)
	...
	browser_backend: Literal['cdp', 'playwright'] = Field(
		default='cdp',
		description='Browser backend: "cdp" (Chromium only, default) or "playwright" (Chromium/Firefox/WebKit).',
	)
	playwright_browser: Literal['chromium', 'firefox', 'webkit'] = Field(
		default='chromium',
		description='When browser_backend=playwright, which browser to use.',
	)
