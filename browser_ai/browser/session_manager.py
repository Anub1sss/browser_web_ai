...
from browser_ai.utils import create_task_with_error_handling

if TYPE_CHECKING:
	from browser_ai.browser.session import BrowserSession, CDPSession, Target
...
class SessionManager:
	...
	async def _handle_target_attached(self, event: AttachedToTargetEvent) -> None:
		...
		from browser_ai.browser.session import CDPSession
	...
	async def _handle_target_detached(self, event: DetachedFromTargetEvent) -> None:
		...
		if target_fully_removed:
			if target_type in ('page', 'tab'):
				from browser_ai.browser.events import TabClosedEvent
	...
	async def _recover_agent_focus(self, crashed_target_id: TargetID) -> None:
		...
		try:
			...
			if page_targets:
				...
				from browser_ai.browser.events import TabCreatedEvent
			...
			if new_session:
				...
				from browser_ai.browser.events import AgentFocusChangedEvent
			...
			for _ in range(20):
				...
				if fallback_session:
					...
					from browser_ai.browser.events import AgentFocusChangedEvent, TabCreatedEvent
