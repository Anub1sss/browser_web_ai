...
from browser_ai.dom.serializer.clickable_elements import ClickableElementDetector
from browser_ai.dom.serializer.paint_order import PaintOrderRemover
from browser_ai.dom.utils import cap_text_length
	...
	def _assign_interactive_indices_and_mark_new_nodes(self, node: SimplifiedNode | None) -> None:
		...
		if not node.excluded_by_parent and not node.ignored_by_paint_order:
			...
			if is_interactive_assign and not node.original_node.snapshot_node:
				...
				logger = logging.getLogger('browser_ai.dom.serializer')
