...
def create_task_with_error_handling(
	...
	*,
	...
	suppress_env = os.getenv('BROWSER_USE_SILENT_BACKGROUND_EXCEPTIONS', '').strip().lower() in {
		'1',
		'true',
		'yes',
		'on',
	}
	should_suppress = suppress_exceptions or suppress_env
	...
	def _handle_task_exception(t: asyncio.Task[T]) -> None:
		...
		if exc_to_raise is not None and not should_suppress:
			raise exc_to_raise
