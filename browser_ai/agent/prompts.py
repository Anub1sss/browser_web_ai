...
from browser_ai.dom.views import NodeType, SimplifiedNode
from browser_ai.llm.messages import ContentPartImageParam, ContentPartTextParam, ImageURL, SystemMessage, UserMessage
from browser_ai.observability import observe_debug
from browser_ai.utils import is_new_tab_page, sanitize_surrogates

if TYPE_CHECKING:
	from browser_ai.agent.views import AgentStepInfo
		...
		try:
			...
			if self.is_browser_use_model:
				if self.flash_mode:
					...
					template_filename = 'system_prompt_browser_ai.md'
				else:
			...
			with (
				importlib.resources.files('browser_ai.agent.system_prompts')
