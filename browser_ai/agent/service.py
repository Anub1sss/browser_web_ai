...
if TYPE_CHECKING:
	from browser_ai.skills.views import Skill

from dotenv import load_dotenv

from browser_ai.agent.cloud_events import (
...
from browser_ai.agent.message_manager.utils import save_conversation
...
from browser_ai.llm.messages import BaseMessage, ContentPartImageParam, ContentPartTextParam, UserMessage
from browser_ai.tokens.service import TokenCost
...
from browser_ai import Browser, BrowserProfile, BrowserSession
from browser_ai.agent.judge import construct_judge_messages

# Lazy import for gif to avoid heavy agent.views import at startup
# from browser_ai.agent.gif import create_history_gif
from browser_ai.agent.message_manager.service import (
	MessageManager,
)
...
from browser_ai.browser.events import _get_timeout
from browser_ai.browser.session import DEFAULT_BROWSER_PROFILE
from browser_ai.browser.views import BrowserStateSummary
from browser_ai.config import CONFIG
from browser_ai.dom.views import DOMInteractedElement, MatchLevel
from browser_ai.filesystem.file_system import FileSystem
from browser_ai.observability import observe, observe_debug
from browser_ai.telemetry.service import ProductTelemetry
from browser_ai.telemetry.views import AgentTelemetryEvent
from browser_ai.tools.registry.views import ActionModel
from browser_ai.tools.service import Tools
from browser_ai.utils import (
	URL_PATTERN,
	_log_pretty_path,
...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		if llm is None:
			...
			if default_llm_name:
				from browser_ai.llm.models import get_llm_by_name

				llm = get_llm_by_name(default_llm_name)
			else:
				# No default LLM specified, use the original default
				from browser_ai import ChatBrowserUse
		...
		if skill_service is not None:
			...
			from browser_ai.skills import SkillService
	...
	def logger(self) -> logging.Logger:
		...
		return logging.getLogger(f'browser_ai.Agent🅰 {_task_id} ⇢ 🅑 {_browser_session_id} 🅣 {_current_target_id}')
	...
	def _set_screenshot_service(self) -> None:
		...
		try:
			from browser_ai.screenshots.service import ScreenshotService
	...
	def _process_messsages_and_replace_long_urls_shorter_ones(self, input_messages: list[BaseMessage]) -> dict[str, str]:
		...
		"""
		from browser_ai.llm.messages import AssistantMessage, UserMessage
	...
	async def _log_agent_run(self) -> None:
		...
		self.logger.debug(f'🤖 Library version {self.version} ({self.source})')
		...
		if CONFIG.BROWSER_USE_VERSION_CHECK:
			...
			if latest_version and latest_version != self.version:
				self.logger.info(
					f'📦 Newer version available: {latest_version} (current: {self.version}). '
					'Upgrade to the latest release via your package manager.'
				)

	def _log_first_step_startup(self) -> None:
		"""Log startup message only on the first step"""
		if len(self.history.history) == 0:
			self.logger.info(f'Starting agent with version {self.version}')
	...
	async def run(
		...
		from browser_ai.utils import SignalHandler
	...
	async def _generate_rerun_summary(
		...
		from browser_ai.agent.views import RerunSummaryAction
		...
		from browser_ai.agent.prompts import get_rerun_summary_message, get_rerun_summary_prompt
		...
		try:
			...
			from browser_ai.llm.messages import BaseMessage
	...
	async def _execute_ai_step(
		...
		"""
		from browser_ai.agent.prompts import get_ai_step_system_prompt, get_ai_step_user_prompt, get_rerun_summary_message
		from browser_ai.llm.messages import SystemMessage, UserMessage
		from browser_ai.utils import sanitize_surrogates
		...
		try:
			from browser_ai.dom.markdown_extractor import extract_clean_markdown
	...
	def detect_variables(self) -> dict[str, DetectedVariable]:
		...
		from browser_ai.agent.variable_detector import detect_variables_in_history
